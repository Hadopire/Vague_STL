...
struct Pair {
    ...
    typename std::enable_if<std::is_default_constructible<TX>::value &&
                            std::is_default_constructible<TY>::value, int>::type = 0>
    Pair() : first(), second() {}
...
private:
    ...
    {
        //std::cout << "resize at load: " << m_load << " maxload: " << m_maxLoad << std::endl;
